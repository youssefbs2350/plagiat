/*
* ============================================================
*  PLAGIAT-DETECTOR V1.0
*  Détection de similarité entre deux fichiers texte
* ============================================================
*
*  PRINCIPE FONCTIONNEL :
*    Comparer deux fichiers texte et calculer leur degré de
*    similarité (plagiat potentiel) via la technique des
*    k-grammes (shingles) et la similarité de Jaccard :
*
*      similarité = |shingles(F1) ∩ shingles(F2)|
*                   ─────────────────────────────  × 100
*                         |shingles(F1)|
*
*    Un shingle = séquence de K=5 mots consécutifs.
*    Si "les processus du systeme sont" apparaît dans F1 ET F2
*    → c'est un passage potentiellement plagié.
*
*  ARCHITECTURE (identique à PARATEXT-ANALYZER V4.3) :
...
#define SHINGLE_K       5        /* k mots par shingle                  */
...
    long  shingles_checked;   /* shingles de F1 analysés par ce worker */
    long  shingles_matched;   /* shingles trouvés dans F2              */
    pid_t pid;
    long  ram_kb;
    float cpu_pct;
} WorkerStats;
...
    uint32_t    ht[HT_SIZE];          /* 2 Mo, open-addressing linéaire */
    long        f2_total_shingles;    /* nb shingles indexés depuis F2  */

 

    /* ── Résultats agrégés (protégés par thr_mutex) ── */
    long        f1_total;             /* shingles de F1 analysés        */
    long        f1_matched;           /* shingles de F1 trouvés dans F2 */

 

    /* ── Passages similaires détectés ── */
    char        passages[MAX_PASSAGES][128];
    int         passage_count;
...
static char            g_file1[256];   /* fichier découpé en tâches  */
static char            g_file2[256];   /* fichier de référence (F2)  */
...
/* ====================================================================
*  HASH TABLE — Empreintes des shingles de F2
*
*  Open-addressing avec sondage linéaire.
*  0 = case vide (les hashes valides ne peuvent pas être 0).
*  La table est construite dans main() AVANT le fork(),
*  puis utilisée en LECTURE SEULE par tous les threads
*  → aucun verrou nécessaire pour les lookups.
* ==================================================================== */
...
*  TOKENISEUR
*
*  Extrait le prochain mot alphanumérique depuis text[*pos..end[
*  en minuscules. Retourne 1 si un mot a été trouvé, 0 sinon.
* ==================================================================== */
...
    return (j >= 3) ? 1 : next_word(text, pos, end, word, maxlen);
}

 

/* ====================================================================
*  CONSTRUCTION DU HASH TABLE depuis F2 (dans main, avant fork)
*
*  Lit F2 entièrement, extrait tous les k-shingles,
*  insère leurs empreintes dans shared_mem->ht.
* ==================================================================== */
...
    /* Fenêtre glissante de SHINGLE_K mots */
...
/* ====================================================================
*  ANALYSE D'UN SEGMENT DE F1
*
*  Extrait les shingles du buffer, les cherche dans le hash table,
*  retourne le nombre de shingles vérifiés et matchés.
*  Sauvegarde jusqu'à MAX_PASSAGES extraits dans shared_mem.
* ==================================================================== */
...
            /* Sauvegarder le passage (section critique légère) */
...
*  BARBIER ENDORMI
*
*  Contrôle l'admission des threads analyseurs.
*  Salle d'attente bornée : si pleine, le thread réessaie.
*  Le barber_thread tourne EN PARALLÈLE des threads workers.
* ==================================================================== */
typedef struct {
...
*
*  5 philosophes, anti-deadlock paire/impaire.
* ==================================================================== */
...
/* ====================================================================
*  THREAD ANALYSEUR — WORK-STEALING
*
*  Chaque thread :
*    1. Entre dans la salle du barbier (admission contrôlée)
*    2. Pioche une tâche dans la file IPC
*    3. Lit le segment de F1, extrait les shingles
*    4. Compare contre le hash table de F2 (lecture seule → sans verrou)
*    5. Met à jour les stats partagées
*    6. Répète jusqu'à la poison pill
* ==================================================================== */
typedef struct {
...
        if (msg.task_id == -1) break;   /* poison pill */
...
        /* Lire le segment de F1 en mémoire */
...
        /* Analyser le chunk → comparer les shingles */
...
    /* Recouvrement : pipe + exec(gzip) → rapport_plagiat.gz */
    int pfd[2];
    if (pipe(pfd) == 0) {
        pid_t gz = fork();
        if (gz == 0) {
...
        }
        close(pfd[0]);
        float pct = (wc > 0) ? (float)wm * 100.0f / (float)wc : 0.0f;
        char rep[256];
        int n = snprintf(rep, sizeof(rep),
            "Worker %d (PID %d): %ld shingles verifies, "
            "%ld matches (%.1f%%), %d taches\n",
...
/* ====================================================================
*  THREAD PRODUCTEUR — Producteur/Consommateur [FIX-40 conservé]
*
*  Remplit la file IPC progressivement PENDANT que les workers
*  consomment → la file ne sature jamais.
* ==================================================================== */
typedef struct {
    long *offsets;
    int   ntasks, nw, nt;
...
    const char *col = (pct > 70.0f) ? "\033[41m" :
                      (pct > 40.0f) ? "\033[43m" : "\033[42m";
    printf("%s", col);
    for (int j=0;j<f;    j++) printf("\xe2\x96\x88");
    printf("\033[40m");
    for (int j=f;j<width;j++) printf("\xe2\x96\x91");
...
    /* Calcul similarité globale */
    long   f1t = shared_mem->f1_total;
    long   f1m = shared_mem->f1_matched;
    float  sim = (f1t > 0) ? (float)f1m * 100.0f / (float)f1t : 0.0f;
...
    printf("\033[1;36m"
"╔══════════════════════════════════════════════════════════════════════════╗\033[K\n"
"║   🔍  PLAGIAT-DETECTOR V1.0 — DÉTECTION DE SIMILARITÉ PARALLÈLE 🔍    ║\033[K\n"
"╚══════════════════════════════════════════════════════════════════════════╝"
"\033[0m\033[K\n");
...
    /* ── Score de similarité ── */
    const char *verdict =
        (sim > 70.0f) ? "\033[1;31m⚠  PLAGIAT PROBABLE\033[0m" :
        (sim > 40.0f) ? "\033[1;33m⚡ SIMILARITÉ ÉLEVÉE\033[0m" :
        (sim > 15.0f) ? "\033[1;34m📝 SIMILARITÉ MODÉRÉE\033[0m" :
                        "\033[1;32m✅ DOCUMENTS DISTINCTS\033[0m";

 

    printf("  \033[1;36m╔══════════════════════════════════════════════════════════╗\033[0m\033[K\n");
    printf("  \033[1;36m║\033[0m  🎯 SIMILARITÉ JACCARD : \033[1;33m%6.2f %%\033[0m  %s",
           sim, verdict);
    printf("  \033[1;36m\033[0m\033[K\n");
    printf("  \033[1;36m║\033[0m  ");
    draw_similarity_bar(sim, 50);
    printf("  \033[1;36m\033[0m\033[K\n");
    printf("  \033[1;36m║\033[0m  Shingles F1 analysés : \033[1;33m%-8ld\033[0m  "
           "Matchés : \033[1;32m%-8ld\033[0m  F2 indexés : \033[1;34m%-8ld\033[0m\033[K\n",
           f1t, f1m, shared_mem->f2_total_shingles);
    printf("  \033[1;36m╚══════════════════════════════════════════════════════════╝\033[0m\033[K\n\n");
...
               "%-*s │\033[K\n", npass-8, (int)(68-35-(npass>99?3:2)), "");
    printf("  └──────────────────────────────────────────────────────────────────┘\033[K\n\n");
...
    printf("  \033[1;36m🔗 Technique: Shingles %d-grammes │ Hash table lock-free │ Jaccard similarity\033[0m\033[K\n",
           SHINGLE_K);
    fflush(stdout);
}
...
            "  Résultat : pourcentage de similarité + passages communs\n\n",
            argv[0], argv[0]);
        return 1;
    }
...
    /* ── IPC_PRIVATE : IDs dynamiques, hérités par fork() ── */
...
    /* Sémaphore nommé unique par PID */
...
    /* ── Étape 1 : Indexer F2 dans le hash table (mémoire partagée) ── */
...
    printf("✅ F2 indexé : %ld shingles (%d-grammes) dans le hash table\n",
           f2_shingles, SHINGLE_K);

 

    /* ── Étape 2 : Calculer les offsets de F1 ── */
    FILE *f = fopen(g_file1, "r");
...
    /* ── Résultat final ── */
    long   tf=shared_mem->f1_total, mf=shared_mem->f1_matched;
    float  sim=(tf>0)?(float)mf*100.0f/(float)tf:0.0f;
    int    npass=shared_mem->passage_count;
...
    printf("\033[?25h\n");
    printf("\033[1;36m══════════════════════════════════════════════\033[0m\n");
    printf("\033[1;32m  RÉSULTAT FINAL\033[0m\n");
    printf("\033[1;36m══════════════════════════════════════════════\033[0m\n");
    printf("  Fichier 1        : %s\n", g_file1);
    printf("  Fichier 2        : %s\n", g_file2);
    printf("  Shingles F1      : %ld\n", tf);
    printf("  Shingles F2 ref  : %ld\n", shared_mem->f2_total_shingles);
    printf("  Matchés          : %ld\n", mf);
    printf("  \033[1;33mSIMILARITÉ      : %.2f %%\033[0m\n", sim);
    if      (sim>70.0f) printf("  \033[1;31mVERDICT : ⚠  PLAGIAT PROBABLE\033[0m\n");
    else if (sim>40.0f) printf("  \033[1;33mVERDICT : ⚡ SIMILARITÉ ÉLEVÉE\033[0m\n");
    else if (sim>15.0f) printf("  \033[1;34mVERDICT : 📝 SIMILARITÉ MODÉRÉE\033[0m\n");
